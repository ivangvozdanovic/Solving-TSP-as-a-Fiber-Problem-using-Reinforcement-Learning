...
'''
def getQvalueES(Q, state, action):
...
'''
def getBestActionES(Q, state, state_edges):
    state_keys = list(Q.keys())
    state_str = ToBytes(state)
    if state_str not in state_keys:
        ...
        #print("State edges: ", state_edges)
        for q in range(len(action_keys)):
            #print("Best action ", FromBytes(max_action_str))
...
def updateQdictES(Q,state,action,update_value):
    state_keys = list(Q.keys())
    state_str = ToBytes(state)
    action_str = ToBytes(action)
    if state_str not in state_keys:
